...
const hikeList = [
  ...
  export default class Hikes {
    ...
    addHikeListener() {
      let arrayImages = Array.from(document.querySelectorAll('.image'));
      arrayImages.forEach(image => {
        image.addEventListener('click', this.showOneHike);
      })
  ...
  function renderOneHikeLight(hike) {
    ...
    <div class="image" data-name ="${hike.name}"><img src="${imgBasePath}${hike.imgSrc}" alt="${hike.imgAlt}"></div>
    <div>
            <div>
                <h3>Distance</h3>
                <p>${hike.distance}</p>
